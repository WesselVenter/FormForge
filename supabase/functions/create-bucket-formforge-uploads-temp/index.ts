...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'formforge-uploads',
        name: 'formforge-uploads',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["image/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/plain"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for formforge-uploads" ON storage.objects FOR SELECT USING (bucket_id = 'formforge-uploads');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for formforge-uploads" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'formforge-uploads');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for formforge-uploads" ON storage.objects FOR UPDATE USING (bucket_id = 'formforge-uploads');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for formforge-uploads" ON storage.objects FOR DELETE USING (bucket_id = 'formforge-uploads');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'formforge-uploads',
        public: true,
        allowed_mime_types: ["image/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/plain"],
